...
const PodcastInfo = ({ podcast }: PodcastInfoProps) => {
  ...
  return <div className='p-4 shadow-md'>
    ...
    <div className='my-3 mx-4'>
      <div data-testid='podcast-title' className='font-bold'>{podcast.title}</div>
      <div data-testid='podcast-author' className='italic'>{t('PODCAST_BY')} {podcast.author}</div>
    </div>
    <div className='my-3 mx-4'>
      <div className='font-bold'>{t('PODCAST_DESCRIPTION')}</div>
      <div data-testid='podcast-description' dangerouslySetInnerHTML={{__html: podcast.description}}></div>
    </div>
  </div>
}
